...
def load_backtest_thresholds(market_name, participant_name):
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
            res = data.get(market_name, {}).get(participant_name, defaults)
            res_copy = dict(res)
            res_copy["percentile_upper"] = 85.0
            res_copy["percentile_lower"] = 15.0
            return res_copy
    except:
        return defaults
...
def calculate_metrics(df):
    ...
    # Group Normalized: relative to group's own total active positions (Long + Short)
    # Bounded strictly between -100% and +100%
    group_total = df["long"] + df["short"]
    df["net_pct_oi"] = np.where(group_total != 0, (df["net"] / group_total) * 100, 0.0)
    df["long_pct_oi"] = np.where(group_total != 0, (df["long"] / group_total) * 100, 0.0)
    df["short_pct_oi"] = np.where(group_total != 0, (df["short"] / group_total) * 100, 0.0)
...
def generate_holistic_report(market_name, use_combined=False):
    ...
    # 3. Историческая эффективность и бэктесты (с учетом лага в 1 неделю)
    insights = []
    
    # Rules based on quantitative research
    if market_name in ["S&P 500", "Nasdaq"]:
        if am_idx >= 85:
            insights.append("* 🟢 **Умные деньги покупают:** Когда институционалы (Asset Managers) заходят в экстремальный лонг (Index >= 85%), исторически цена на горизонте 12 недель росла в **71.3%** случаев (средний прирост **+2.49%**). За ними выгодно повторять покупки.")
        elif am_idx <= 15:
            insights.append("* 🟢 **Умные деньги в максимальной панике:** При экстремальном шорте институционалов (Index <= 15%) исторически наблюдался мощный разворот вверх: Win Rate роста составлял **87.2%** через 4 недели (средний прирост **+4.07%**) и **84.6%** через 12 недель (средний прирост **+5.52%**).")
            
        if lf_idx <= 15:
            insights.append("* 🟢 **Капитуляция спекулянтов:** Хедж-фонды (Leveraged Funds) находятся на пике коротких позиций (Index <= 15%). Исторически это сильный контр-индикатор: рынок разворачивался вверх с Win Rate **59.7%** (4н) и **64.5%** (12н) из-за шорт-сквиза.")
        elif lf_idx >= 85:
            insights.append("* 🔴 **Перегрев спекулянтов:** Хедж-фонды в экстремальном лонге (Index >= 85%). Бэктесты показывают Win Rate дальнейшего падения в **64.9%** случаев на горизонте 4-8 недель.")
            
    elif market_name == "EUR":
        # Check weekly flow direction for Leveraged Funds
        lf_wow = latest_lf.get("wow_change_net_pct_oi", 0)
        if lf_wow > 0:
            insights.append("* 🟢 **Трендовый импульс хедж-фондов:** Хедж-фонды на EUR выступают сильным трендовым индикатором. Их импульсные покупки (Wow Chg >= 80th pct) исторически показывают Win Rate роста **54.8%** на горизонте 4 недель.")
        elif lf_wow < 0:
            insights.append("* 🔴 **Медвежий импульс спекулянтов:** Импульс продаж хедж-фондов на EUR исторически имеет высокую вероятность продолжения падения на горизонте 4-12 недель.")
            
    elif market_name == "BTC":
        if rt_idx >= 85:
            insights.append("* 🔴 **Толпа на хаях:** Розничные спекулянты (Retail) на BTC зашли в экстремальный лонг (Index >= 85%). Исторически это идеальный контр-индикатор: цена падала или корректировалась в **58.8%** случаев на горизонте 4 недель. Стоит воздержаться от покупок.")
        elif rt_idx <= 15:
            insights.append("* 🟢 **Паника толпы:** Розничные спекулянты в максимальном шорте (Index <= 15%). Это исторический сигнал на покупку с Win Rate роста **52.4%**.")
            
        if lf_idx >= 85:
            insights.append("* 🚀 **Сильный бычий тренд хедж-фондов:** Хедж-фонды на BTC зашли в экстремальный лонг (Index >= 85%). Бэктест показывает, что в этих точках цена **всегда росла** на горизонте 12 недель (Win Rate **100%**, средний прирост **+39.0%**). Это признак мощного долгосрочного аптренда.")

    if insights:
        insights_str = "**📈 Исторические закономерности и вероятности (бэктест с лагом 1 нед.):**\n" + "\n".join(insights)
    else:
        insights_str = "**📈 Исторические закономерности и вероятности:** Актуальных аномалий с высокой статистической вероятностью разворота/тренда по бэктесту для текущих позиций не обнаружено."
