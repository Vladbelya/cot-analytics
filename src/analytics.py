...
def generate_holistic_report(market_name, use_combined=False):
    ...
    if market_name in ["S&P 500", "Nasdaq"]:
        if am_idx >= 90:
            insights.append("* 🟢 **Умные деньги покупают:** Когда институционалы (Asset Managers) заходят в экстремальный лонг (Index >= 90%), исторически цена на горизонте 12 недель росла в **71.3%** случаев (средний прирост **+2.49%**). За ними выгодно повторять покупки.")
        elif am_idx <= 10:
            insights.append("* 🟢 **Умные деньги в максимальной панике:** При экстремальном шорте институционалов (Index <= 10%) исторически наблюдался мощный разворот вверх: Win Rate роста составлял **87.2%** через 4 недели (средний прирост **+4.07%**) и **84.6%** через 12 недель (средний прирост **+5.52%**).")
            
        if lf_idx <= 10:
            insights.append("* 🟢 **Капитуляция спекулянтов:** Хедж-фонды (Leveraged Funds) находятся на пике коротких позиций (Index <= 10%). Исторически это сильный контр-индикатор: рынок разворачивался вверх с Win Rate **59.7%** (4н) и **64.5%** (12н) из-за шорт-сквиза.")
        elif lf_idx >= 90:
            insights.append("* 🔴 **Перегрев спекулянтов:** Хедж-фонды в экстремальном лонге (Index >= 90%). Бэктесты показывают Win Rate дальнейшего падения в **64.9%** случаев на горизонте 4-8 недель.")
        ...
        if rt_idx >= 90:
            insights.append("* 🔴 **Толпа на хаях:** Розничные спекулянты (Retail) на BTC зашли в экстремальный лонг (Index >= 90%). Исторически это идеальный контр-индикатор: цена падала или корректировалась в **58.8%** случаев на горизонте 4 недель. Стоит воздержаться от покупок.")
        elif rt_idx <= 10:
            insights.append("* 🟢 **Паника толпы:** Розничные спекулянты в максимальном шорте (Index <= 10%). Это исторический сигнал на покупку с Win Rate роста **52.4%**.")
            
        if lf_idx >= 90:
            insights.append("* 🚀 **Сильный бычий тренд хедж-фондов:** Хедж-фонды на BTC зашли в экстремальный лонг (Index >= 90%). Бэктест показывает, что в этих точках цена **всегда росла** на горизонте 12 недель (Win Rate **100%**, средний прирост **+39.0%**). Это признак мощного долгосрочного аптренда.")
